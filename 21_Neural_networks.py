...
for item in x_test:
    ...
    h2_in = np.dot(h1_out, w1) + b1  # this calculates the linear combination of inputs and weights
    h2_out = hidden_activation(h2_in)

    h3_in = np.dot(h2_out, w2) + b2  # this calculates the linear combination of inputs and weights
    h3_out = hidden_activation(h3_in)
